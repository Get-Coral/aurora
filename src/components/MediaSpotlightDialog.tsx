...
export function MediaSpotlightDialog({
	...
	const detail: DetailedMediaItem = (activeItem.type === "series"
		? seriesData?.item
		: data?.item) ?? { ...activeItem, cast: [], studios: [], tags: [] };
	const similar =
		activeItem.type === "series" ? (seriesData?.similar ?? []) : (data?.similar ?? []);
	...
	const metadata = [detail.year, formatRuntime(detail.runtimeMinutes), detail.ageRating].filter(
		Boolean,
	);
	...
	const playLabel =
		activeItem.type === "series"
			? isResumable(nextUp[0] ?? episodes[0] ?? detail)
				? t("hero.resumeNow")
				: t("hero.playNow")
			: resumable
				? t("hero.resumeNow")
				...
				<div className="tv-detail-content page-wrap">
					<div className="tv-detail-copy">
						{detail.logoUrl ? (
							...
							<h2 id="aurora-dialog-title" className="tv-detail-title">
								{detail.title}
							</h2>
						)}
						...
						{detail.overview ? <p className="tv-detail-overview">{detail.overview}</p> : null}
					...
					<div className="tv-detail-lower">
						{item.type === "series" ? (
							<div className="tv-episodes-section">
								...
								{seriesLoading ? (
									<p className="tv-detail-section-label" style={{ opacity: 0.5 }}>
										{t("dialog.loadingCredits")}
									</p>
											...
											return (
												<button
													...
													onClick={() =>
														onPlay?.(
															episode,
															globalIdx >= 0 ? episodes.slice(globalIdx) : [episode],
														)
													}
												>
													<div className="tv-episode-thumb-wrap">
														...
														{episode.played ? (
															...
															<div className="tv-episode-progress">
																<div
																	className="tv-episode-progress-fill"
													...
													<div className="tv-episode-info">
														<span className="tv-episode-num">
															...
															{isNextUp ? (
																<span className="tv-episode-next-badge">
																	{t("dialog.nextUpReady")}
																</span>
															) : null}
														</span>
								...
								<div className="tv-detail-scroll-row">
									{similar.slice(0, 12).map((similarItem) => (
										<button
											...
											onMouseEnter={() =>
												void prefetchMediaDetails(similarItem).catch(() => undefined)
											}
										...
										>
											...
											<span>
												{[similarItem.year, similarItem.ageRating].filter(Boolean).join(" • ")}
											</span>
										</button>
									))}
								</div>
				...
				<div className="dialog-body">
					<div className="dialog-header-block">
						...
						<div className="dialog-copy-block">
							<p className="eyebrow">
								{detail.type === "series"
									? t("dialog.seriesSpotlight")
									: t("dialog.movieSpotlight")}
							</p>
							...
							<p className="dialog-overview">{detail.overview || t("dialog.noSynopsis")}</p>
				...
				<div className="dialog-lower">
							...
							{nextUp.length ? (
								<div className="episode-highlight">
									...
									>
										...
										{isResumable(nextUp[0]) ? t("dialog.resumeNextUp") : t("dialog.playNextUp")}
									</button>
								</div>
							) : null}
								...
								<div className="episode-list">
									{episodes.slice(0, 20).map((episode) => {
										...
										return (
											<div
												key={episode.id}
												className={`episode-row${isPlayed ? " episode-row-watched" : ""}`}
											>
												<button
													type="button"
												...
												>
													...
													<span className="episode-progress">
														{isPlayed
															...
															: episode.progress
																? t("hero.progressWatched", {
																		progress: Math.round(episode.progress),
																	})
																: isResumable(episode)
																	? t("card.resume")
																	: t("dialog.playEpisode")}
												...
												<button
													...
													aria-label={
														isPlayed ? t("dialog.markUnwatched") : t("dialog.markWatched")
													}
												>
					...
					<section className="detail-section">
						<div className="detail-section-head">
							...
							{loadingState ? (
								<span className="detail-loading">{t("dialog.loadingCredits")}</span>
							) : null}
						</div>
						{detail.cast.length ? (
