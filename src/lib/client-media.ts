...
export async function createClientAdminUser(input: {
	name: string;
	password: string;
	...
	return {
		...
		streamUrl:
			session.playMethod === "Transcode"
				? setTranscodeQuality(session.streamUrl)
				: session.streamUrl,
	};
}

export async function reportClientPlaybackState(input: { id: string; played?: boolean }) {
	if (input.played) {
...
export async function searchClientOnlineSubtitles(input: {
	...
	return json.data
		...
		.map((subtitle) => ({
			...
			label:
				LANGUAGE_NAMES[subtitle.attributes.language] ?? subtitle.attributes.language.toUpperCase(),
