    ...
    <main className="library-shell">
              ...
              {!tvMode ? (
                <div className="collection-card-actions">
                  ...
                  <button
                    ...
                    onClick={() => setDialog({ type: 'delete', id: collection.id, name: collection.title })}
      ...
      {dialog?.type === 'create' ? (
        <div className="coll-dialog-backdrop" onClick={() => setDialog(null)} role="presentation">
          <div className="coll-dialog" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
            <div className="coll-dialog-head">
              <h2>{t('route.collections.newTitle')}</h2>
              <button type="button" className="icon-button" onClick={() => setDialog(null)}><X size={16} /></button>
            </div>
            ...
            <div className="coll-dialog-footer">
              ...
              >
                <Plus size={14} />
                {createMutation.isPending ? t('route.collections.creating') : t('route.collections.newSubmit')}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="coll-dialog" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
            ...
            <label className="library-select-shell" style={{ width: '100%' }}>
              ...
              <input
                ...
                value={nameInput}
                onChange={(e) => setNameInput(e.target.value)}
                onKeyDown={(e) => e.key === 'Enter' && nameInput.trim() && renameMutation.mutate({ id: dialog.id })}
              />
            </label>
            ...
            <div className="coll-dialog-footer">
              ...
              <button
                type="button"
                className="primary-action"
                disabled={!nameInput.trim() || nameInput === dialog.currentName || renameMutation.isPending}
                onClick={() => renameMutation.mutate({ id: dialog.id })}
              >
                {renameMutation.isPending ? t('route.collections.saving') : t('route.collections.editSubmit')}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="coll-dialog-backdrop" onClick={() => setDialog(null)} role="presentation">
          <div className="coll-dialog" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
            ...
            <div className="coll-dialog-footer">
              ...
              >
                <Trash2 size={14} />
                {deleteMutation.isPending ? t('route.collections.deleting') : t('route.collections.delete')}
              </button>
            </div>
          </div>
        </div>
